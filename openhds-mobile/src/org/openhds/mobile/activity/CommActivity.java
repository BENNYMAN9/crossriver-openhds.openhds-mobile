...
/**
 * Community Activity mediates the interaction between the 3 column fragments. The
 * buttons in the left most column drive a state machine while the user
 * interacts with the application.
 */
public class CommActivity extends Activity implements ValueFragment.ValueListener, LoaderCallbacks<Cursor>,
    ...
    /**
     * This differs from {@link org.openhds.mobile.activity.CommActivity.CheckFormStatus} in that, upon
     ...
     * status is set to {@link org.openhds.mobile.InstanceProviderAPI.STATUS_COMPLETE}. If the user
    ...
    private void createInMigrationFormDialog() {
        ...
        alertDialogBuilder.setNegativeButton("External", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                showProgressFragment();
                getLoaderManager().restartLoader(SOCIAL_GROUP_FOR_EXT_INMIGRATION, null, CommActivity.this);
            }
        });
    ...
    private class PregnancyOutcomeTask extends AsyncTask<Void, Void, Individual> {
        ...
        protected void onPostExecute(final Individual father) {
            ...
            AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(CommActivity.this);
    ...
    public void loadForm(final int requestCode) {
        new OdkGeneratedFormLoadTask(getContentResolver(), filledForm, new OdkFormLoadListener() {
            public void onOdkFormLoadSuccess(Uri contentUri) {
                CommActivity.this.contentUri = contentUri;
